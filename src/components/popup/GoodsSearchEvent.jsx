...
const GoodsSearchEvent = ({ onGoodsSelected }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await request(
        ...
        {
          action: "selectEventProduct",
          payload: {
            ...filters,
            searchUserId: user?.agentId ?? "", // ✅ fallback 제거
            agentId,
          },
        },
        {},
  ...
  const columnDefs = useMemo(() => {
    const columns = [
      ...
      gridNoColumn(),
      { headerName: "상품코드", field: "goodsId", width: 150, cellClass: "text-center", flex: 1 },
      { headerName: "상품명", field: "goodsNm", width: 200, cellClass: "text-left", flex: 1 },
      {
        headerName: "등록일자", field: "openD", width: 120, cellClass: "text-center", flex: 1,
...
export default React.memo(GoodsSearchEvent);
